...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sheetIframe = document.querySelector('#sheet-widget-body iframe');

    function applyThemeToSheets(theme) {
        if (!sheetIframe) return;
        if (theme === 'dark') {
            sheetIframe.style.filter = 'invert(0.9) hue-rotate(180deg) contrast(1.1)';
        } else {
            sheetIframe.style.filter = 'none';
        }
    }

    const currentTheme = localStorage.getItem('theme') || 'dark';
    document.documentElement.setAttribute('data-theme', currentTheme);
    applyThemeToSheets(currentTheme);
